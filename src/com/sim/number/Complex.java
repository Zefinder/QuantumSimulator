 ...
 * equivalent method in ISO C99. The behavior for special cases is listed as
 * defined in C99.
 * </p>
 *
 * <p>
 * For functions \( f \) which obey the conjugate equality \( conj(f(z)) =
 * f(conj(z)) \), the specifications for the upper half-plane imply the
 * specifications for the lower half-plane.
 * </p>
 *
 * <p>
 * For functions that are either odd, \( f(z) = -f(-z) \), or even, \( f(z) =
 * f(-z) \), the specifications for the first quadrant imply the specifications
 * for the other three quadrants.
 * </p>
 *
 * <p>
 * Special cases of <a href="http://mathworld.wolfram.com/BranchCut.html">branch
 * cuts</a> for multivalued functions adopt the principle value convention from
 * C99. Specials cases from C99 that raise the "invalid" or "divide-by-zero"
 * <a href=
 * "https://en.cppreference.com/w/c/numeric/fenv/FE_exceptions">floating-point
 * exceptions</a> return the documented value without an explicit mechanism to
 * notify of the exception case, that is no exceptions are thrown during
 * computations in-line with the convention of the corresponding single-valued
 * functions in {@link Math Math}. These cases are documented in the method
 * special cases as "invalid" or "divide-by-zero" floating-point operation.
 * Note: Invalid floating-point exception cases will result in a complex number
 * where the cardinality of NaN component parts has increased as a real or
 * imaginary part could not be computed and is set to NaN.
 *
...
public final class Complex implements Serializable, Cloneable {
	...
	/**
	 * {@code 1.0 / sqrt(2)}. This is pre-computed to the closest double from the
	 * exact result. It is 1 ULP different from 1.0 / Math.sqrt(2) but equal to
	 * Math.sqrt(2) / 2.
	 */
	 ...
	 *
	 * <p>
	 * In IEEE 754 arithmetic, this is 2<sup>-53</sup>. Copied from
	 * o.a.c.numbers.Precision.
	 *
	...
	/**
	 * The multiplier used to split the double value into hi and low parts. This
	 * must be odd and a value of 2^s + 1 in the range {@code p/2 <= s <= p-1} where
	 * p is the number of bits of precision of the floating point number. Here
	 * {@code s = 27}.
	 */
	private static final double MULTIPLIER = 1.34217729E8;

	/**
	 * Crossover point to switch computation for asin/acos factor A. This has been
	 * updated from the 1.5 value used by Hull et al to 10 as used in
	 * boost::math::complex.
	 * 
	 * @see <a href="https://svn.boost.org/trac/boost/ticket/7290">Boost ticket
	 *      7290</a>
	 */
	 ...
	 * asin/acos. Equal to sqrt(M) / 8 in Hull, et al (1997) with M the largest
	 * normalised floating-point value.
	 */
	private static final double SAFE_MAX = Math.sqrt(Double.MAX_VALUE) / 8;
	/**
	 * The safe minimum double value {@code x} to avoid loss of precision/underflow
	 * in asin/acos. Equal to sqrt(u) * 4 in Hull, et al (1997) with u the smallest
	 * normalised floating-point value.
	 */
	...
	/**
	 * The safe minimum double value {@code x} to avoid loss of precision/underflow
	 * in atanh. Equal to sqrt(u) * 2 with u the smallest normalised floating-point
	 * value.
	 */
	...
	/**
	 * A safe maximum double value {@code m} where {@code e^m} is not infinite. This
	 * can be used when functions require approximations of sinh(x) or cosh(x) when
	 * x is large using exp(x):
	 * 
	 * <pre>
	 ...
	 *
	 * <p>
	 * The value should be below ln(max_value) ~ 709.783. The value m is set to an
	 * integer for less error when subtracting m and chosen as even (m=708) as it is
	 * used as a threshold in tanh with m/2.
	 *
	 * <p>
	 * The value is used to compute e^x multiplied by a small number avoiding
	 * overflow (sinh/cosh) or a small number divided by e^x without underflow due
	 * to infinite e^x (tanh). The following conditions are used:
	 * 
	 * <pre>
	...
	/**
	 * The value of Math.exp(SAFE_EXP): e^708. To be used in overflow/underflow safe
	 * products of e^m to approximate e^x where {@code x > m}.
	 */
	private static final double EXP_M = Math.exp(SAFE_EXP);

	/**
	 * 54 shifted 20-bits to align with the exponent of the upper 32-bits of a
	 * double.
	 */
	private static final int EXP_54 = 0x36_00000;
	/**
	 * Represents an exponent of 500 in unbiased form shifted 20-bits to align with
	 * the upper 32-bits of a double.
	 */
	private static final int EXP_500 = 0x5f3_00000;
	/**
	 * Represents an exponent of 1024 in unbiased form (infinite or nan) shifted
	 * 20-bits to align with the upper 32-bits of a double.
	 */
	...
	/**
	 * The minimum number of characters in the format. This is 5, e.g.
	 * {@code "(0,0)"}.
	 */
	...
	/**
	 * The minimum number of characters before the separator. This is 2, e.g.
	 * {@code "(0"}.
	 */
	...
	/**
	 * Define a constructor for a Complex. This is used in functions that implement
	 ...
	 * {@code rho} (\( \rho \)) and phase angle {@code theta} (\( \theta \)).
	 *
	 * \[ \begin{aligned} x &amp;= \rho \cos(\theta) \\ y &amp;= \rho \sin(\theta)
	 * \end{aligned} \]
	 *
	 * <p>
	 * Requires that {@code rho} is non-negative and non-NaN and {@code theta} is
	 * finite; otherwise returns a complex with NaN real and imaginary parts. A
	 * {@code rho} value of {@code -0.0} is considered negative and an invalid
	 * modulus.
	 *
	 * <p>
	 * A non-NaN complex number constructed using this method will satisfy the
	 * following to within floating-point error when {@code theta} is in the range
	 * \( -\pi\ \lt \theta \leq \pi \):
	 *
	 * <pre>
	 ...
	 *
	 * <ul>
	 * <li>{@code ofPolar(}\( -0.0 \){@code , }\( 0 \){@code ) = }\( \text{NaN} + i
	 * \text{NaN} \)
	 * <li>{@code ofPolar(}\( 0.0 \){@code , }\( 0 \){@code ) = }\( 0 + i 0 \)
	 * <li>{@code ofPolar(}\( 1 \){@code , }\( 0 \){@code ) = }\( 1 + i 0 \)
	 * <li>{@code ofPolar(}\( 1 \){@code , }\( \pi \){@code ) = }\( -1 + i \sin(\pi)
	 * \)
	 * <li>{@code ofPolar(}\( \infty \){@code , }\( \pi \){@code ) = }\( -\infty + i
	 * \infty \)
	 * <li>{@code ofPolar(}\( \infty \){@code , }\( 0 \){@code ) = }\( -\infty + i
	 * \text{NaN} \)
	 ...
	 * {@link #toString() Complex.toString()}. The format expects a start and end
	 * parentheses surrounding two numeric parts split by a separator. Leading and
	 * trailing spaces are allowed around each numeric part. Each numeric part is
	 * parsed using {@link Double#parseDouble(String)}. The parts are interpreted as
	...
	public static Complex parse(String s) {
		...
		if (len < FORMAT_MIN_LEN) {
			throw new NumberFormatException(parsingExceptionMsg("Input too short, expected format",
					FORMAT_START + "x" + FORMAT_SEP + "y" + FORMAT_END, s));
		}

		// Confirm start: '('
		if (s.charAt(0) != FORMAT_START) {
			throw new NumberFormatException(parsingExceptionMsg("Expected start delimiter", FORMAT_START, s));
		}

		// Confirm end: ')'
		if (s.charAt(len - 1) != FORMAT_END) {
			throw new NumberFormatException(parsingExceptionMsg("Expected end delimiter", FORMAT_END, s));
		}
		...
		if (s.indexOf(FORMAT_SEP, sep + 1) != -1) {
			throw new NumberFormatException(
					parsingExceptionMsg("Incorrect number of parts, expected only 2 using separator", FORMAT_SEP, s));
		}
		...
		try {
			...
			throw new NumberFormatException(parsingExceptionMsg("Could not parse real part", rePart, s));
		}
		...
		try {
			...
			throw new NumberFormatException(parsingExceptionMsg("Could not parse imaginary part", imPart, s));
		}
	...
	private static String parsingExceptionMsg(String message, Object error, String s) {
		final StringBuilder sb = new StringBuilder(100).append(message).append(" '").append(error)
	...
	/**
	 * Returns the absolute value of this complex number. This is also called
	 * complex norm, modulus, or magnitude.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The cases ensure that if either component is infinite then the result is
	 * positive infinity. If either component is NaN and this is not
	 * {@link #isInfinite() infinite} then the result is NaN.
	 *
	 * <p>
	 * This method follows the
	 * <a href="http://www.iso-9899.info/wiki/The_Standard">ISO C Standard</a>,
	 * Annex G, in calculating the returned value without intermediate overflow or
	 * underflow.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * This should satisfy the special cases of the hypot function in ISO C99
	 * F.9.4.3: "The hypot functions compute the square root of the sum of the
	 * squares of x and y, without undue overflow or underflow."
	 *
	 * <ul>
	 ...
	 *
	 * <p>
	 * This method is called by all methods that require the absolute value of the
	 * complex number, e.g. abs(), sqrt() and log().
	 *
	 * @param real      Real part.
	 * @param imaginary Imaginary part.
	 * @return The absolute value.
	 ...
	 * \( -\pi \) (not inclusive) and \( \pi \) (inclusive), with negative values
	 ...
	 * coordinate and returning a multiple of \( \frac{\pi}{4} \) depending on the
	 * signs of the infinite parts.
	 *
	 * <p>
	 * This code follows the
	 * <a href="http://www.iso-9899.info/wiki/The_Standard">ISO C Standard</a>,
	 * Annex G, in calculating the returned value using the {@code atan2(y, x)}
	 * method for complex \( x + iy \).
	 *
	...
	/**
	 * Returns the squared norm value of this complex number. This is also called
	 * the absolute square.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * If either component is infinite then the result is positive infinity. If
	 * either component is NaN and this is not {@link #isInfinite() infinite} then
	 * the result is NaN.
	 *
	 * <p>
	 * Note: This method may not return the same value as the square of
	 * {@link #abs()} as that method uses an extended precision computation.
	 *
	 * <p>
	 * {@code norm()} can be used as a faster alternative than {@code abs()} for
	 * ranking by magnitude. If used for ranking any overflow to infinity will
	 * create an equal ranking for values that may be still distinguished by
	 * {@code abs()}.
	 *
	 ...
	 * @see #isNaN()
	 * @see #abs()
	 * @see <a href="http://mathworld.wolfram.com/AbsoluteSquare.html">Absolute
	 *      square</a>
	 */
	public double norm() {
		if (isInfinite()) {
	...
	/**
	 * Returns {@code true} if either the real <em>or</em> imaginary component of
	 * the complex number is NaN <em>and</em> the complex number is not infinite.
	 *
	 * <p>
	 * Note that:
	 * <ul>
	...
	/**
	 * Returns {@code true} if either real or imaginary component of the complex
	 * number is infinite.
	 *
	 * <p>
	 * Note: A complex number with at least one infinite part is regarded as an
	...
	/**
	 * Returns {@code true} if both real and imaginary component of the complex
	 * number are finite.
	 *
	 ...
	 *
	 * <p>
	 * \[ \begin{aligned} z &amp;= a + i b \\ \overline{z} &amp;= a - i b
	 * \end{aligned}\]
	 *
	...
	/**
	 * Returns a {@code Complex} whose value is the negation of both the real and
	 ...
	 * real axis.
	 *
	 * If \( z \) has an infinite part, then {@code z.proj()} shall be equivalent
	 * to:
	 *
	 * <pre>
	...
	/**
	 * Returns a {@code Complex} whose value is {@code (this + addend)}. Implements
	 * the formula:
	 *
	 * <p>
	 ...
	 * @see <a href="http://mathworld.wolfram.com/ComplexAddition.html">Complex
	 *      Addition</a>
	 */
	 ...
	 * {@code addend} interpreted as a real number. Implements the formula:
	 *
	 * <p>
	 * \[ (a + i b) + c = (a + c) + i b \]
	 *
	 * <p>
	 * This method is included for compatibility with ISO C99 which defines
	 * arithmetic between real-only and complex numbers.
	 * </p>
	 *
	 * <p>
	 * Note: This method preserves the sign of the imaginary component \( b \) if it
	 * is {@code -0.0}. The sign would be lost if adding \( (c + i 0) \) using
	...
	/**
	 * Returns a {@code Complex} whose value is {@code (this + addend)}, with
	 * {@code addend} interpreted as an imaginary number. Implements the formula:
	 *
	 * <p>
	 * \[ (a + i b) + i d = a + i (b + d) \]
	 *
	 * <p>
	 * This method is included for compatibility with ISO C99 which defines
	 * arithmetic between imaginary-only and complex numbers.
	 * </p>
	 *
	 * <p>
	 * Note: This method preserves the sign of the real component \( a \) if it is
	 * {@code -0.0}. The sign would be lost if adding \( (0 + i d) \) using
	 ...
	 * @see <a href="http://mathworld.wolfram.com/ComplexSubtraction.html">Complex
	 *      Subtraction</a>
	 */
	...
	/**
	 * Returns a {@code Complex} whose value is {@code (this - subtrahend)}, with
	 * {@code subtrahend} interpreted as a real number. Implements the formula:
	 *
	 * <p>
	 * \[ (a + i b) - c = (a - c) + i b \]
	 *
	...
	/**
	 * Returns a {@code Complex} whose value is {@code (this - subtrahend)}, with
	 * {@code subtrahend} interpreted as an imaginary number. Implements the
	 * formula:
	 *
	 * <p>
	 * \[ (a + i b) - i d = a + i (b - d) \]
	...
	/**
	 * Returns a {@code Complex} whose value is {@code (minuend - this)}, with
	 * {@code minuend} interpreted as a real number. Implements the formula: \[ c -
	 * (a + i b) = (c - a) - i b \]
	 *
	 * <p>
	 * This method is included for compatibility with ISO C99 which defines
	 ...
	 *
	 * <p>
	 * Note: This method inverts the sign of the imaginary component \( b \) if it
	 * is {@code 0.0}. The sign would not be inverted if subtracting from \( c + i 0
	 * \) using {@link #subtract(Complex) Complex.ofCartesian(minuend,
	 * 0).subtract(this)} since {@code 0.0 - 0.0 = 0.0}.
	 *
	...
	/**
	 * Returns a {@code Complex} whose value is {@code (this - subtrahend)}, with
	 * {@code minuend} interpreted as an imaginary number. Implements the formula:
	 * \[ i d - (a + i b) = -a + i (d - b) \]
	 *
	 * <p>
	 * This method is included for compatibility with ISO C99 which defines
	 ...
	 * {@code 0.0}. The sign would not be inverted if subtracting from \( 0 + i d \)
	 * using {@link #subtract(Complex) Complex.ofCartesian(0,
	 * minuend).subtract(this)} since {@code 0.0 - 0.0 = 0.0}.
	 *
	 * @param minuend Value this complex number is to be subtracted from.
	 * @return {@code this - subtrahend}.
	 * @see #subtract(Complex)
	 ...
	 * @see <a href=
	 *      "http://mathworld.wolfram.com/ComplexMultiplication.html">Complex
	 *      Muliplication</a>
	 */
	 ...
	 * returned as zero. The sign is maintained.
	 *
	 * <pre>
	 *  inf  =  1
	 * -inf  = -1
	 ...
	 * {@code factor} interpreted as a real number. Implements the formula:
	 *
	 * <p>
	 * \[ (a + i b) c = (ac) + i (bc) \]
	 *
	 ...
	 *
	 * <p>
	 * Note: This method should be preferred over using {@link #multiply(Complex)
	 * multiply(Complex.ofCartesian(factor, 0))}. Multiplication can generate signed
	 * zeros if either {@code this} complex has zeros for the real and/or imaginary
	 * component, or if the factor is zero. The summation of signed zeros in
	 * {@link #multiply(Complex)} may create zeros in the result that differ in sign
	...
	/**
	 * Returns a {@code Complex} whose value is {@code this * factor}, with
	 * {@code factor} interpreted as an imaginary number. Implements the formula:
	 *
	 * <p>
	 * \[ (a + i b) id = (-bd) + i (ad) \]
	 *
	 * <p>
	 * This method can be used to compute the multiplication of this complex number
	 * \( z \) by \( i \) using a factor with magnitude 1.0. This should be used in
	 * preference to {@link #multiply(Complex) multiply(Complex.I)} with or without
	 * {@link #negate() negation}:
	 * </p>
	 *
	 ...
	 * <p>
	 * Note: This method should be preferred over using {@link #multiply(Complex)
	 * multiply(Complex.ofCartesian(0, factor))}. Multiplication can generate signed
	 * zeros if either {@code this} complex has zeros for the real and/or imaginary
	 * component, or if the factor is zero. The summation of signed zeros in
	 * {@link #multiply(Complex)} may create zeros in the result that differ in sign
	 ...
	 *
	 * <p>
	 * Re-calculates NaN result values to recover infinities as specified in C99
	 * standard G.5.1.
	 *
	 * @param divisor Value by which this complex number is to be divided.
	 * @return {@code this / divisor}.
	 * @see <a href="http://mathworld.wolfram.com/ComplexDivision.html">Complex
	 *      Division</a>
	 */
	 ...
	 *
	 * <p>
	 * Recalculates to recover infinities as specified in C99 standard G.5.1. Method
	 * is fully in accordance with C++11 standards for complex numbers.
	 * </p>
	 *
	 * <p>
	 * Note: In the event of divide by zero this method produces the same result as
	 * dividing by a real-only zero using {@link #divide(double)}.
	 *
	 * @param re1 Real component of first number.
	 * @param im1 Imaginary component of first number.
	 * @param re2 Real component of second number.
	 ...
	 * {@code divisor} interpreted as a real number. Implements the formula:
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Note: This method should be preferred over using {@link #divide(Complex)
	 * divide(Complex.ofCartesian(divisor, 0))}. Division can generate signed zeros
	 * if {@code this} complex has zeros for the real and/or imaginary component, or
	 * the divisor is infinite. The summation of signed zeros in
	 * {@link #divide(Complex)} may create zeros in the result that differ in sign
	...
	/**
	 * Returns a {@code Complex} whose value is {@code (this / divisor)}, with
	 * {@code divisor} interpreted as an imaginary number. Implements the formula:
	 *
	 * <p>
	 ...
	 * <p>
	 * Note: This method should be preferred over using {@link #divide(Complex)
	 * divide(Complex.ofCartesian(0, divisor))}. Division can generate signed zeros
	 * if {@code this} complex has zeros for the real and/or imaginary component, or
	 * the divisor is infinite. The summation of signed zeros in
	 * {@link #divide(Complex)} may create zeros in the result that differ in sign
	 ...
	 * {@link #divide(Complex) divide(Complex.ofCartesian(0, divisor))} if the
	 * divisor is zero. In this case the divide method using a zero-valued Complex
	 * will produce the same result as dividing by a real-only zero. The output from
	 * dividing by imaginary zero will create infinite and NaN values in the same
	 * component parts as the output from
	 * {@code this.divide(Complex.ZERO).multiplyImaginary(1)}, however the sign of
	 * some infinite values may be negated.
	 *
	 * @param divisor Value by which this complex number is to be divided.
	 * @return {@code this / divisor}.
	 * @see #divide(Complex)
	...
	/**
	 * Returns the <a href="http://mathworld.wolfram.com/ExponentialFunction.html">
	 ...
	 *
	 * <p>
	 * The exponential function of \( z \) is an entire function in the complex
	 * plane. Special cases:
	 *
	 * <ul>
	 ...
	 * <li>If {@code z} is x + iNaN for finite x, returns NaN + iNaN ("invalid"
	 * floating-point operation).
	 * <li>If {@code z} is +∞ + i0, returns +∞ + i0.
	 * <li>If {@code z} is −∞ + iy for finite y, returns +0 cis(y) (see
	 * {@link #ofCis(double)}).
	 * <li>If {@code z} is +∞ + iy for finite nonzero y, returns +∞ cis(y).
	 * <li>If {@code z} is −∞ + i∞, returns ±0 ± i0 (where the signs of the real and
	 * imaginary parts of the result are unspecified).
	 * <li>If {@code z} is +∞ + i∞, returns ±∞ + iNaN (where the sign of the real
	 * part of the result is unspecified; "invalid" floating-point operation).
	 * <li>If {@code z} is −∞ + iNaN, returns ±0 ± i0 (where the signs of the real
	 * and imaginary parts of the result are unspecified).
	 * <li>If {@code z} is +∞ + iNaN, returns ±∞ + iNaN (where the sign of the real
	...
	public Complex exp() {
		if (Double.isInfinite(real)) {
			...
			return new Complex(zeroOrInf * Math.cos(imaginary), zeroOrInf * Math.sin(imaginary));
		} else if (Double.isNaN(real)) {
			// (NaN + i0) returns (NaN + i0)
			// (NaN + iy) returns (NaN + iNaN) and optionally raises the invalid
	...
	/**
	 * Returns the <a href="http://mathworld.wolfram.com/NaturalLogarithm.html">
	 * natural logarithm</a> of this complex number.
	 *
	 * <p>
	 * The natural logarithm of \( z \) is unbounded along the real axis and in the
	 * range \( [-\pi, \pi] \) along the imaginary axis. The imaginary part of the
	 * natural logarithm has a branch cut along the negative real axis \( (-infty,0]
	 * \). Special cases:
	 *
	 * <ul>
	 * <li>{@code z.conj().log() == z.log().conj()}.
	 * <li>If {@code z} is −0 + i0, returns −∞ + iπ ("divide-by-zero" floating-point
	 * operation).
	 * <li>If {@code z} is +0 + i0, returns −∞ + i0 ("divide-by-zero" floating-point
	 * operation).
	 ...
	 * </p>
	 * <blockquote> T E Hull, Thomas F Fairgrieve and Ping Tak Peter Tang (1994)
	 * Implementing complex elementary functions using exception handling. ACM
	 * Transactions on Mathematical Software, Vol 20, No 2, pp 215-244.
	 * </blockquote>
	 *
	...
	/**
	 * Returns the base 10
	 * <a href="http://mathworld.wolfram.com/CommonLogarithm.html"> common
	 * logarithm</a> of this complex number.
	 *
	 * <p>
	 * The common logarithm of \( z \) is unbounded along the real axis and in the
	 * range \( [-\pi, \pi] \) along the imaginary axis. The imaginary part of the
	 ...
	 *
	 * <p>
	 * Warning: The argument {@code logOf2} must be equal to {@code log(2)} using
	 * the provided log function otherwise scaling using powers of 2 in the case of
	 * overflow will be incorrect. This is provided as an internal optimisation.
	 *
	 ...
	 * {@code x}. Implements the formula:
	 *
	 * <p>
	 * \[ z^x = e^{x \ln(z)} \]
	 *
	 * <p>
	 * If this complex number is zero then this method returns zero if {@code x} is
	 * positive in the real component and zero in the imaginary component; otherwise
	 * it returns NaN + iNaN.
	 *
	 ...
	 * @see #exp()
	 * @see <a href=
	 *      "http://mathworld.wolfram.com/ComplexExponentiation.html">Complex
	 *      exponentiation</a>
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/Power/">Power</a>
	 */
	...
	/**
	 * Returns the complex power of this complex number raised to the power of
	 * {@code x}, with {@code x} interpreted as a real number. Implements the
	 * formula:
	 *
	 * <p>
	 * \[ z^x = e^{x \ln(z)} \]
	 ...
	 * <p>
	 * If this complex number is zero then this method returns zero if {@code x} is
	 * positive; otherwise it returns NaN + iNaN.
	 *
	 * @param x The exponent to which this complex number is to be raised.
	 * @return This complex number raised to the power of {@code x}.
	 * @see #log()
	...
	/**
	 * Returns the <a href="http://mathworld.wolfram.com/SquareRoot.html"> square
	 * root</a> of this complex number.
	 *
	 * <p>
	 * \[ \sqrt{x + iy} = \frac{1}{2} \sqrt{2} \left( \sqrt{ \sqrt{x^2 + y^2} + x }
	 * + i\ \text{sgn}(y) \sqrt{ \sqrt{x^2 + y^2} - x } \right) \]
	 *
	 * <p>
	 * The square root of \( z \) is in the range \( [0, +\infty) \) along the real
	 * axis and is unbounded along the imaginary axis. The imaginary part of the
	 * square root has a branch cut along the negative real axis \( (-infty,0) \).
	 * Special cases:
	 *
	 * <ul>
	 ...
	 * <li>\( \text{sgn}(y) =\ \){@link Math#copySign(double,double) copySign}(1.0,
	 * y)
	 * </ul>
	 *
	 * <p>
	 * The implementation is overflow and underflow safe based on the method
	 * described in:
	 * </p>
	 * <blockquote> T E Hull, Thomas F Fairgrieve and Ping Tak Peter Tang (1994)
	 * Implementing complex elementary functions using exception handling. ACM
	 ...
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/Sqrt/">Sqrt</a>
	 */
	public Complex sqrt() {
	...
	private static Complex sqrt(double real, double imaginary) {
		...
		if (inRegion(x, y, Double.MIN_NORMAL, SQRT_SAFE_UPPER)) {
			...
			if (isPosInfinite(y)) {
				...
				// Note if we scale by an even power of 2, we can re-scale by sqrt of the
				// number.
				// a = sqrt(b)
				// a = sqrt(b/4) * sqrt(4)
	...
	/**
	 * Returns the <a href="http://mathworld.wolfram.com/Sine.html"> sine</a> of
	 * this complex number.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * This is an odd function: \( \sin(z) = -\sin(-z) \). The sine is an entire
	 * function and requires no branch cuts.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * As per the C99 standard this function is computed using the trigonomic
	 * identity:
	 *
	 * <p>
	 * \[ \sin(z) = -i \sinh(iz) \]
	...
	/**
	 * Returns the <a href="http://mathworld.wolfram.com/Cosine.html"> cosine</a> of
	 * this complex number.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * This is an even function: \( \cos(z) = \cos(-z) \). The cosine is an entire
	 * function and requires no branch cuts.
	 *
	 * <p>
	 * This is implemented using real \( x \) and imaginary \( y \) parts:
	...
	/**
	 * Returns the <a href="http://mathworld.wolfram.com/Tangent.html"> tangent</a>
	 ...
	 *
	 * <p>
	 * This is an odd function: \( \tan(z) = -\tan(-z) \). The tangent is an entire
	 * function and requires no branch cuts.
	 *
	 * <p>
	 * This is implemented using real \( x \) and imaginary \( y \) parts:
	 * </p>
	 * \[ \tan(x + iy) = \frac{\sin(2x)}{\cos(2x)+\cosh(2y)} + i
	 ...
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/Tan/">Tangent</a>
	 */
	public Complex tan() {
	...
	/**
	 * Returns the <a href="http://mathworld.wolfram.com/InverseSine.html"> inverse
	 ...
	 *
	 * <p>
	 * The inverse sine of \( z \) is unbounded along the imaginary axis and in the
	 * range \( [-\pi, \pi] \) along the real axis. Special cases are handled as if
	 * the operation is implemented using \( \sin^{-1}(z) = -i \sinh^{-1}(iz) \).
	 *
	 * <p>
	 * The inverse sine is a multivalued function and requires a branch cut in the
	 * complex plane; the cut is conventionally placed at the line segments \(
	 ...
	 *
	 * <p>
	 * \[ \begin{aligned} \sin^{-1}(z) &amp;= \sin^{-1}(B) + i\ \text{sgn}(y)\ln
	 * \left(A + \sqrt{A^2-1} \right) \\ A &amp;= \frac{1}{2} \left[
	 * \sqrt{(x+1)^2+y^2} + \sqrt{(x-1)^2+y^2} \right] \\ B &amp;= \frac{1}{2}
	 * \left[ \sqrt{(x+1)^2+y^2} - \sqrt{(x-1)^2+y^2} \right] \end{aligned} \]
	 *
	 * <p>
	 ...
	 * The implementation is based on the method described in:
	 * </p>
	 * <blockquote> T E Hull, Thomas F Fairgrieve and Ping Tak Peter Tang (1997)
	 * Implementing the complex Arcsine and Arccosine Functions using Exception
	 * Handling. ACM Transactions on Mathematical Software, Vol 23, No 3, pp
	 * 299-335. </blockquote>
	 *
	 * <p>
	 * The code has been adapted from the <a href="https://www.boost.org/">Boost</a>
	 * {@code c++} implementation {@code <boost/math/complex/asin.hpp>}.
	 *
	 * @return The inverse sine of this complex number.
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/ArcSin/">ArcSin</a>
	 */
	public Complex asin() {
	 ...
	 *
	 * <p>
	 * Adapted from {@code <boost/math/complex/asin.hpp>}. This method only (and not
	 * invoked methods within) is distributed under the Boost Software License V1.0.
	 * The original notice is shown below and the licence is shown in full in
	 * LICENSE:
	 * 
	 * <pre>
	...
	/**
	 * Returns the <a href="http://mathworld.wolfram.com/InverseCosine.html">
	 ...
	 *
	 * <p>
	 * The inverse cosine of \( z \) is in the range \( [0, \pi) \) along the real
	 * axis and unbounded along the imaginary axis. Special cases:
	 *
	 * <ul>
	 ...
	 * <li>If {@code z} is x + iNaN, returns NaN + iNaN ("invalid" floating-point
	 * operation).
	 ...
	 *
	 * <p>
	 * The inverse cosine is a multivalued function and requires a branch cut in the
	 * complex plane; the cut is conventionally placed at the line segments \(
	 * (-\infty,-1) \) and \( (1,\infty) \) of the real axis.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * \[ \begin{aligned} \cos^{-1}(z) &amp;= \cos^{-1}(B) - i\ \text{sgn}(y)
	 * \ln\left(A + \sqrt{A^2-1}\right) \\ A &amp;= \frac{1}{2} \left[
	 * \sqrt{(x+1)^2+y^2} + \sqrt{(x-1)^2+y^2} \right] \\ B &amp;= \frac{1}{2}
	 * \left[ \sqrt{(x+1)^2+y^2} - \sqrt{(x-1)^2+y^2} \right] \end{aligned} \]
	 *
	 * <p>
	 ...
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/ArcCos/">ArcCos</a>
	 */
	public Complex acos() {
	 ...
	 *
	 * <p>
	 * Adapted from {@code <boost/math/complex/acos.hpp>}. This method only (and not
	 * invoked methods within) is distributed under the Boost Software License V1.0.
	 * The original notice is shown below and the licence is shown in full in
	 * LICENSE:
	 * 
	...
	/**
	 * Returns the <a href="http://mathworld.wolfram.com/InverseTangent.html">
	 ...
	 *
	 * <p>
	 * The inverse hyperbolic tangent of \( z \) is unbounded along the imaginary
	 * axis and in the range \( [-\pi/2, \pi/2] \) along the real axis.
	 *
	 * <p>
	 ...
	 * <p>
	 * As per the C99 standard this function is computed using the trigonomic
	 * identity: \[ \tan^{-1}(z) = -i \tanh^{-1}(iz) \]
	 *
	 * @return The inverse tangent of this complex number.
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/ArcTan/">ArcTan</a>
	 */
	public Complex atan() {
	 ...
	 *
	 * <p>
	 * The hyperbolic sine of \( z \) is an entire function in the complex plane and
	 * is periodic with respect to the imaginary component with period \( 2\pi i \).
	 * Special cases:
	 *
	 * <ul>
	 ...
	 * <li>If {@code z} is +0 + i0, returns +0 + i0.
	 * <li>If {@code z} is +0 + i∞, returns ±0 + iNaN (where the sign of the real
	 * part of the result is unspecified; "invalid" floating-point operation).
	 * <li>If {@code z} is +0 + iNaN, returns ±0 + iNaN (where the sign of the real
	 * part of the result is unspecified).
	 ...
	 * ("invalid" floating-point operation).
	 * <li>If {@code z} is +∞ + i0, returns +∞ + i0.
	 * <li>If {@code z} is +∞ + iy for positive finite y, returns +∞ cis(y) (see
	 * {@link #ofCis(double)}.
	 * <li>If {@code z} is +∞ + i∞, returns ±∞ + iNaN (where the sign of the real
	 * part of the result is unspecified; "invalid" floating-point operation).
	 * <li>If {@code z} is +∞ + iNaN, returns ±∞ + iNaN (where the sign of the real
	 ...
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/Sinh/">Sinh</a>
	 */
	public Complex sinh() {
	...
	private static Complex sinh(double real, double imaginary, ComplexConstructor constructor) {
		...
		if (real == 0) {
			...
			// It is not possible to also maintain the odd function (hence the unspecified
			// sign).
			return constructor.create(real, Double.NaN);
		}
		if (imaginary == 0) {
	 ...
	 * ("invalid" floating-point operation).
	 * <li>If {@code z} is +∞ + i0, returns +∞ + i0.
	 * <li>If {@code z} is +∞ + iy for finite nonzero y, returns +∞ cis(y) (see
	 * {@link #ofCis(double)}).
	 * <li>If {@code z} is +∞ + i∞, returns ±∞ + iNaN (where the sign of the real
	 * part of the result is unspecified).
	 * <li>If {@code z} is +∞ + iNaN, returns +∞ + iNaN.
	 ...
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/Cosh/">Cosh</a>
	 */
	public Complex cosh() {
	...
	private static Complex cosh(double real, double imaginary, ComplexConstructor constructor) {
		...
		if (real == 0) {
			...
			// Although not required by C99 changing the sign maintains the conjugate
			// equality.
			// It is not possible to also maintain the even function (hence the unspecified
			// sign).
		...
		if (imaginary == 0) {
			...
			// => change the sign of imaginary using real. Handles special case of infinite
			// real.
	...
	/**
	 * Compute cosh or sinh when the absolute real component |x| is large. In this
	 * case cosh(x) and sinh(x) can be approximated by exp(|x|) / 2:
	 *
	 * <pre>
	...
	/**
	 * Returns the <a href="http://mathworld.wolfram.com/HyperbolicTangent.html">
	 ...
	 * \( (0, \pi(\frac{1}{2} + n)) \). Note that the {@code double} floating-point
	 * representation is unable to exactly represent \( \pi/2 \) and there is no
	 * value for which a pole error occurs. Special cases:
	 *
	 * <ul>
	 ...
	 * ("invalid" floating-point operation).
	 * <li>If {@code z} is +∞ + iy for positive-signed finite y, returns 1 + i0
	 * sin(2y).
	 * <li>If {@code z} is +∞ + i∞, returns 1 ± i0 (where the sign of the imaginary
	 * part of the result is unspecified).
	 ...
	 *
	 * <p>
	 * \[ \tan(x + iy) = \frac{\sinh(2x)}{\cosh(2x)+\cos(2y)} + i
	 ...
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/Tanh/">Tanh</a>
	 */
	public Complex tanh() {
	...
	private static Complex tanh(double real, double imaginary, ComplexConstructor constructor) {
		...
		if (!isPosFinite(x) || !Double.isFinite(imaginary)) {
			if (isPosInfinite(x)) {
				if (Double.isFinite(imaginary)) {
					...
					final double sign = Math.abs(imaginary) < PI_OVER_2 ? imaginary
		...
		// To avoid a junction when swapping between the double angles and the
		// identities
	...
	/**
	 * Returns the
	 * <a href="http://mathworld.wolfram.com/InverseHyperbolicSine.html"> inverse
	 * hyperbolic sine</a> of this complex number.
	 *
	 * <p>
	 ...
	 * ("invalid" floating-point operation).
	 * <li>If {@code z} is NaN + i∞, returns ±∞ + iNaN (where the sign of the real
	 * part of the result is unspecified).
	 * <li>If {@code z} is NaN + iNaN, returns NaN + iNaN.
	 * </ul>
	 *
	 * <p>
	 * The inverse hyperbolic sine is a multivalued function and requires a branch
	 * cut in the complex plane; the cut is conventionally placed at the line
	 * segments \( (-i \infty,-i) \) and \( (i,i \infty) \) of the imaginary axis.
	 *
	 * <p>
	 ...
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/ArcSinh/">ArcSinh</a>
	 */
	...
	/**
	 * Returns the
	 * <a href="http://mathworld.wolfram.com/InverseHyperbolicCosine.html"> inverse
	 * hyperbolic cosine</a> of this complex number.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The inverse hyperbolic cosine of \( z \) is in the range \( [0, \infty) \)
	 * along the real axis and in the range \( [-\pi, \pi] \) along the imaginary
	 * axis. Special cases:
	 *
	 * <ul>
	 ...
	 *
	 * <p>
	 * The inverse hyperbolic cosine is a multivalued function and requires a branch
	 * cut in the complex plane; the cut is conventionally placed at the line
	 * segment \( (-\infty,-1) \) of the real axis.
	 *
	 * <p>
	 * This function is computed using the trigonomic identity:
	 *
	 ...
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/ArcCosh/">ArcCosh</a>
	 */
	...
	/**
	 * Returns the
	 * <a href="http://mathworld.wolfram.com/InverseHyperbolicTangent.html"> inverse
	 * hyperbolic tangent</a> of this complex number.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The inverse hyperbolic tangent of \( z \) is unbounded along the real axis
	 * and in the range \( [-\pi/2, \pi/2] \) along the imaginary axis. Special
	 * cases:
	 *
	 * <ul>
	 ...
	 * <li>If {@code z} is +0 + i0, returns +0 + i0.
	 * <li>If {@code z} is +0 + iNaN, returns +0 + iNaN.
	 * <li>If {@code z} is +1 + i0, returns +∞ + i0 ("divide-by-zero" floating-point
	 * operation).
	 * <li>If {@code z} is x + i∞ for finite positive-signed x, returns +0 + iπ/2.
	 * <li>If {@code z} is x+iNaN for nonzero finite x, returns NaN+iNaN ("invalid"
	 * floating-point operation).
	 ...
	 * <li>If {@code z} is NaN+iy for finite y, returns NaN+iNaN ("invalid"
	 * floating-point operation).
	 * <li>If {@code z} is NaN + i∞, returns ±0 + iπ/2 (where the sign of the real
	 * part of the result is unspecified).
	 * <li>If {@code z} is NaN + iNaN, returns NaN + iNaN.
	 * </ul>
	 *
	 * <p>
	 * The inverse hyperbolic tangent is a multivalued function and requires a
	 * branch cut in the complex plane; the cut is conventionally placed at the line
	 * segments \( (\infty,-1] \) and \( [1,\infty) \) of the real axis.
	 *
	 * <p>
	 * This is implemented using real \( x \) and imaginary \( y \) parts:
	 *
	 ...
	 * \\ i \frac{1}{2} \left( \tan^{-1} \left(\frac{2y}{1-x^2-y^2} \right) +
	 * \frac{\pi}{2} \left(\text{sgn}(x^2+y^2-1)+1 \right) \text{sgn}(y) \right) \]
	 *
	 * <p>
	 * The imaginary part is computed using {@link Math#atan2(double, double)} to
	 * ensure the correct quadrant is returned from \( \tan^{-1}
	 * \left(\frac{2y}{1-x^2-y^2} \right) \).
	 *
	 * <p>
	 * The code has been adapted from the <a href="https://www.boost.org/">Boost</a>
	 ...
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/ArcTanh/">ArcTanh</a>
	 */
	 ...
	 *
	 * <p>
	 * Adapted from {@code <boost/math/complex/atanh.hpp>}. This method only (and
	 * not invoked methods within) is distributed under the Boost Software License
	 * V1.0. The original notice is shown below and the licence is shown in full in
	 * LICENSE:
	 * 
	 * <pre>
	 * (C) Copyright John Maddock 2005.
	...
	/**
	 * Compute {@code x^2 + y^2 - 1} in high precision. Assumes that the values x
	 * and y can be multiplied without overflow; that {@code x >= y}; and both
	 * values are positive.
	 *
	 * @param x the x value
	 * @param y the y value
	...
	private static double x2y2m1(double x, double y) {
		...
		// If 4 y^2 < |x^2 – 1| the cancellation is not serious ... the argument is
		// accurate
		// only to within a factor of 1 + 4 EPSILSON so that error = 4 EPSILON.

		// Otherwise there can be serious cancellation and the relative error in the
		// real part
		// could be enormous.
		...
		// Set the threshold when x^2 + y^2 is above 0.5 thus subtracting 1 results in a
		// number
		// that can be expressed with a higher precision than any number in the range
		// 0.5-1.0
		...
		if (x < 1 && xx + yy > 0.5) {
			...
			// - the final term is -1 (which does not require split multiplication and
			// addition)
	...
	/**
	 * Implement Dekker's method to split a value into two parts. Multiplying by
	 * (2^s + 1) create a big value from which to derive the two split parts.
	 * 
	 * <pre>
	 * c = (2^s + 1) * a
	 * a_big = c - a
	 ...
	 * {@code a_lo}. Combined they have (p􏰔-1) bits of significand but the sign bit
	 * of {@code a_lo} contains a bit of information.
	 *
	 * @param a Value.
	 * @return the high part of the value.
	 * @see <a href="https://doi.org/10.1007/BF01397083"> Dekker (1971) A
	 *      floating-point technique for extending the available precision</a>
	 */
	 ...
	 * {@code high} components. Uses Dekker's algorithm for split multiplication
	 * modified for a square product.
	 *
	 * <p>
	 * Note: This is candidate to be replaced with {@code Math.fma(x, x, -x * x)} to
	 * compute the round-off from the square product {@code x * x}. This would
	 * remove the requirement to compute the split number and make this method
	 * redundant. {@code Math.fma} requires JDK 9 and FMA hardware support.
	 *
	...
	/**
	 * Compute the round-off from the sum of two numbers {@code a} and {@code b}
	 * using Dekker's two-sum algorithm. The values are required to be ordered by
	 * magnitude: {@code |a| >= |b|}.
	 *
	...
	/**
	 * Compute the round-off from the sum of two numbers {@code a} and {@code b}
	 * using Knuth's two-sum algorithm. The values are not required to be ordered by
	 * magnitude.
	 *
	 * @param a First part of sum.
	 * @param b Second part of sum.
	 ...
	 *
	 * <p>
	 * Implement Shewchuk's expansion-sum algorithm: [x2Low, x2High] + [-1] +
	 * [y2Low, y2High].
	 *
	...
	private static double sumx2y2m1(double x2High, double x2Low, double y2High, double y2Low) {
		...
		// Expansion-sum proceeds by a grow-expansion of the first part from one
		// expansion
		// into the other, extending its length by 1. The process repeats for the next
		// part
		// but the grow-expansion starts at the previous merge position + 1.
		// Thus expansion-sum requires mn two-sum operations to merge length m into
		// length n
		// resulting in length m+n-1.

		// Variables numbered from 1 as per Figure 7 (p.12). The output expansion h is
		// placed
		...
		// representations. This is then added to a similar magnitude y^2. Leaving the
		// -1
		...
		// Note: Do not use the alternative fast-expansion-sum of the parts sorted by
		// magnitude.
		// The parts can be ordered with a single comparison into:
		// [y2Low, (y2High|x2Low), x2High, -1]
		// The fast-two-sum saves 1 fast-two-sum and 3 two-sum operations (21 additions)
		// and
		// adds a penalty of a single branch condition.
		// However the order in not "strongly non-overlapping" and the
		// fast-expansion-sum
		// output will not be strongly non-overlapping. The sum of the output has 1 ulp
		// error
		// on random cis numbers approximately 1 in 160 events. This can be removed by a
		// distillation two-sum pass over the final expansion as a cost of 1
		// fast-two-sum and
		// 3 two-sum operations! So we use the expansion sum with the same operations
		// and
		// no branches.
		...
		// |e - sum| < ulp(sum).
		// To achieve the exact result requires iteration of a distillation two-sum
		// through
	...
	/**
	 * Returns the n-th roots of this complex number. The nth roots are defined by
	 * the formula:
	 *
	 * <p>
	 * \[ z_k = |z|^{\frac{1}{n}} \left( \cos \left(\phi + \frac{2\pi k}{n} \right)
	 * + i \sin \left(\phi + \frac{2\pi k}{n} \right) \right) \]
	 *
	 * <p>
	 * for \( k=0, 1, \ldots, n-1 \), where \( |z| \) and \( \phi \) are
	 * respectively the {@link #abs() modulus} and {@link #arg() argument} of this
	 * complex number.
	 *
	 * <p>
	 * If one or both parts of this complex number is NaN, a list with all all
	 * elements set to {@code NaN + i NaN} is returned.
	 * </p>
	 *
	 ...
	 * @see <a href=
	 *      "http://functions.wolfram.com/ElementaryFunctions/Root/">Root</a>
	 */
	...
	/**
	 * Test for equality with another object. If the other object is a
	 * {@code Complex} then a comparison is made of the real and imaginary parts;
	 * otherwise {@code false} is returned.
	 *
	 * <p>
	 * If both the real and imaginary parts of two complex numbers are exactly the
	 * same the two {@code Complex} objects are considered to be equal. For this
	 * purpose, two {@code double} values are considered to be the same if and only
	 * if the method {@link Double #doubleToLongBits(double)} returns the identical
	 * {@code long} value when applied to each.
	 *
	 * <p>
	 * Note that in most cases, for two instances of class {@code Complex},
	 * {@code c1} and {@code c2}, the value of {@code c1.equals(c2)} is {@code true}
	 * if and only if
	 *
	 * <pre>
	 *  {@code
	 ...
	 *
	 * <ul>
	 * <li>Instances that contain {@code NaN} values in the same part are considered
	 * to be equal for that part, even though {@code Double.NaN == Double.NaN} has
	 * the value {@code false}.</li>
	 * <li>Instances that share a {@code NaN} value in one part but have different
	 * values in the other part are <em>not</em> considered equal.</li>
	 * <li>Instances that contain different representations of zero in the same part
	 * are <em>not</em> considered to be equal for that part, even though
	 * {@code -0.0 == 0.0} has the value {@code true}.</li>
	 * </ul>
	 *
	 * <p>
	 * The behavior is the same as if the components of the two complex numbers were
	 * passed to {@link java.util.Arrays#equals(double[], double[])
	 ...
	 *
	 * <p>
	 * The behavior is the same as if the components of the complex number were
	 * passed to {@link java.util.Arrays#hashCode(double[])
	 ...
	 * The values are split by a separator and surrounded by parentheses. The string
	 * can be {@link #parse(String) parsed} to obtain an instance with the same
	 * value.
	 *
	 * <p>
	 * The format for complex number \( x + i y \) is {@code "(x,y)"}, with \( x \)
	 * and \( y \) converted as if using {@link Double#toString(double)}.
	 *
	...
	/**
	 * Check that a value is positive infinity. Used to replace
	 * {@link Double#isInfinite()} when the input value is known to be positive
	 * (i.e. in the case where it has been set using {@link Math#abs(double)}).
	 *
	 * @param d Value.
	...
	/**
	 * Check that an absolute value is finite. Used to replace
	 * {@link Double#isFinite(double)} when the input value is known to be positive
	 * (i.e. in the case where it has been set using {@link Math#abs(double)}).
	 *
	 * @param d Value.
	 ...
	 * {@code -i}. This is used in functions that implement trigonomic identities.
	 ...
	 *
	 * <p>
	 * If the signed value is negative then the result is {@code -magnitude};
	 * otherwise return {@code magnitude}.
	 *
	 * <p>
	 * A signed value of {@code -0.0} is treated as negative. A signed value of
	 * {@code NaN} is treated as positive.
	 *
	 * <p>
	...
	/**
	 * Returns a scale suitable for use with {@link Math#scalb(double, int)} to
	 * normalise the number to the interval {@code [1, 2)}.
	 *
	 * <p>
	 * The scale is typically the largest unbiased exponent used in the
	 * representation of the two numbers. In contrast to
	 * {@link Math#getExponent(double)} this handles sub-normal numbers by computing
	 * the number of leading zeros in the mantissa and shifting the unbiased
	 * exponent. The result is that for all finite, non-zero, numbers {@code a, b},
	 * the magnitude of {@code scalb(x, -getScale(a, b))} is always in the range
	 * {@code [1, 2)}, where {@code x = max(|a|, |b|)}.
	 *
	 * <p>
	 * This method is a functional equivalent of the c function ilogb(double)
	 * adapted for two input arguments.
	 *
	 * <p>
	 * The result is to be used to scale a complex number using
	 * {@link Math#scalb(double, int)}. Hence the special case of both zero
	 * arguments is handled using the return value for NaN as zero cannot be scaled.
	 * This is different from {@link Math#getExponent(double)} or
	 * {@link #getMaxExponent(double, double)}.
	 *
	 * <p>
	 * Special cases:
	 *
	...
	/**
	 * Returns the largest unbiased exponent used in the representation of the two
	 ...
	 *
	 * <p>
	 * This is used by {@link #divide(double, double, double, double)} as a simple
	 * detection that a number may overflow if multiplied by a value in the interval
	 * [1, 2).
	 *
	 * @param a the first value
	 * @param b the second value
	 ...
	 * will be differences between this method and
	 * {@code Math.hypot(double, double)} due to the use of a different algorithm to
	 * compute the high precision sum of {@code x^2 + y^2}. This method has been
	 * tested to have a lower maximum error from the exact result; any differences
	 * are expected to be 1 ULP indicating a rounding change in the sum.
	 *
	 * <p>
	 * JDK9 ported the hypot function to Java for bug JDK-7130085 due to the slow
	 * performance of the method as a native function. Benchmarks of the Complex
	 * class for functions that use hypot confirm this is slow pre-Java 9. This
	 * implementation outperforms the new faster {@code Math.hypot(double, double)}
	 * on JDK 11 (LTS). See the Commons numbers examples JMH module for benchmarks.
	 * Comparisons with alternative implementations indicate performance gains are
	 * related to edge case handling and elimination of an unpredictable branch in
	 * the computation of {@code x^2 + y^2}.
	 *
	 * <p>
	 * This port was adapted from the "Freely Distributable Math Library" hypot
	 * function. This method only (and not invoked methods within) is distributed
	 ...
	 *
	 * <p>
	 * Note: The fdlibm c code makes use of the language ability to read and write
	 * directly to the upper and lower 32-bits of the 64-double. The function
	 * performs checking on the upper 32-bits for the magnitude of the two numbers
	 * by accessing the exponent and 20 most significant bits of the mantissa. These
	 * upper bits are manipulated during scaling and then used to perform extended
	 * precision computation of the sum {@code x^2 + y^2} where the high part of the
	 * number has 20-bit precision. Manipulation of direct bits has no equivalent in
	 * Java other than use of {@link Double#doubleToLongBits(double)} and
	 * {@link Double#longBitsToDouble(long)}. To avoid conversion to and from long
	 * and double representations this implementation only scales the double
	 * representation. The high and low parts of a double for the extended precision
	 * computation are extracted using the method of Dekker (1971) to create two
	 * 26-bit numbers. This works for sub-normal numbers and reduces the maximum
	 * error in comparison to fdlibm hypot which does not use a split number
	 * algorithm for sub-normal numbers.
	 *
	 * @param x Value x
	 * @param y Value y
	 * @return sqrt(x^2 + y^2)
	 ...
	 * @see <a href=
	 *      "https://bugs.java.com/bugdatabase/view_bug.do?bug_id=7130085">JDK-7130085
	...
	private static double hypot(double x, double y) {
		...
		// This invalidates hypot(x, y) = hypot(y, x) for small sub-normal numbers and a
		// minority
		...
		// precision sum x^2 + y^2. Since exponent scaling cannot be applied to
		// sub-normals
		...
		// insensitive to the sign bit. Thus use of Math.abs(double) is only in
		// edge-case
		// branches.
		//
		// 5. The exponent different to ignore the smaller component has changed from 60
		// to 54.
		//
	...
	private static double x2y2(double x, double y) {
		// Note:
		// This method is different from the high-accuracy summation used in fdlibm for
		// hypot.
		// The summation could be any valid computation of x^2+y^2. However since this
		// follows
		...
		// With FMA hardware support in JDK 9+ this can be replaced with the much
		// faster:
